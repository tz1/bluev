...
ISR(TIMER1_CAPT_vect)
{
    ...
    lastedge = thisedge;
    TCCR1B ^= _BV(ICES1);
    polarity = !(TCCR1B &_BV(ICES1));

#if 1
// Legacy Mode - needs pulldown or need to use AIN0 for threshold.
#define LEGABIT (504)
#define USTICS(us) ((us) * (F_CPU/PRESCALE1)/1000000)
    if( legacy > 100 ) {
static unsigned long legimg;
char legbits[36] = "+medcbap87654321gfKALFFFSSSRRRIXMNOP";
	UCSRB |= _BV(TXEN); // TX on
	    ...
	    inmsglen = 0;
            cks += inmsgbuf[inmsglen++] = 0xaa; // SOF
            cks += inmsgbuf[inmsglen++] = 0xd8; // Dest - broadcast
            cks += inmsgbuf[inmsglen++] = 0xea; // Source - V1
            cks += inmsgbuf[inmsglen++] = 0x31; // infDisplay
            cks += inmsgbuf[inmsglen++] = 9; // Len
            ...
            cks += inmsgbuf[inmsglen++] = 0xe | ((legimg>>31) & 1);
            inmsgbuf[inmsglen++] = 0;
            inmsgbuf[inmsglen++] = 0;

            inmsgbuf[inmsglen++] = cks; // checksum 
            inmsgbuf[inmsglen++] = 0xab; // EOF
	    //	    inmsgstate = 4; // FIXME need to refactor for this to work
        }
        return;
    }
    // FIXME - fails on 4313, works on mega
    if(width > USTICS(LEGABIT) - 25 && width < USTICS(LEGABIT) + 25 && legacy < 200) { //legacy edge to edge
	legacy++;
	UCSRB |= _BV(TXEN); // TX on
        UDR = 'L' + polarity;
    }
...
#include <util/setbaud.h>
    ...
    TCCR1B = _BV(ICNC1) | _BV(CS11) | _BV(ICES1);
